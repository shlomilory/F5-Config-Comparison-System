...
def normalize_site_ip(ip_str: str) -> Tuple[str, str]:
    ...
    Examples:
    10.100.50.10:443 -> (SITE.50.10:443, 'SITE1')
    10.200.50.10:443 -> (SITE.50.10:443, 'SITE2')
    192.168.1.1:80   -> (192.168.1.1:80, 'OTHER')
    """
    # Extract IP pattern
    site1_pattern = r'10\.100\.(\d+\.\d+)'
    site2_pattern = r'10\.200\.(\d+\.\d+)'

    site1_match = re.search(site1_pattern, ip_str)
    site2_match = re.search(site2_pattern, ip_str)

    if site1_match:
        host_part = site1_match.group(1)
        normalized = re.sub(site1_pattern, f'SITE.{host_part}', ip_str)
        return (normalized, 'SITE1')
    elif site2_match:
        host_part = site2_match.group(1)
        normalized = re.sub(site2_pattern, f'SITE.{host_part}', ip_str)
        return (normalized, 'SITE2')
    else:
        return (ip_str, 'OTHER')
...
def classify_ip_difference(ip1: str, ip2: str) -> Tuple[bool, str]:
    ...
    Severity levels:
    - 'MATCH': Same host across sites (10.100.x.x vs 10.200.x.x with same host portion)
    - 'WARNING': Cross-site different hosts (10.100.x.x vs 10.200.y.y)
    - 'CRITICAL': Same-network different hosts (10.100.x.x vs 10.100.y.y) OR completely different networks
    """
    ...
    # Both are site IPs
    # Both are site IPs
    if site1 in ['SITE1', 'SITE2'] and site2 in ['SITE1', 'SITE2']:
    # CRITICAL: Same network, different hosts (both SITE1 OR both SITE2)
        if site1 == site2:
            return (True, 'CRITICAL')
    # WARNING: Cross-site, different hosts (SITE1 vs SITE2)
        else:
            return (True, 'WARNING')
...
<body>
    <div class="header">
        ...
        <div class="header-meta">
            <div><strong>Site 1:</strong> {server1}</div>
            <div><strong>Site 2:</strong> {server2}</div>
        ...
        function renderComparisons() {{
            ...
            comparisonData.forEach((vs) => {{
                ...
                if (vs.hasNoRedundancy) {{
                    const missingSite = vs.missingInFile1 ? 'Site 1' : 'Site 2';
                    redundancyWarning = `
                        <div class="redundancy-warning">
                            <strong>⚠️ No Redundancy:</strong> This virtual server only exists in ${{vs.missingInFile1 ? 'Site 2' : 'Site 1'}}. Missing in ${{missingSite}}.
                        </div>
                    `;
                }}
...
def lambda_handler(event, context):
    ...
    config_path = event.get('config_path', os.environ.get('CONFIG_PATH', '/config/bigip.conf'))
